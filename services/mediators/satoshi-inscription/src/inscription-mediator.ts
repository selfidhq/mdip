...
async function fetchBlock(height: number, blockCount: number): Promise<void> {
  try {
    ...
    const block = (await btcClient.getBlock(
      blockHash,
      BlockVerbosity.JSON_TX_DATA
    )) as BlockVerbose;
...
async function scanBlocks(): Promise<void> {
  ...
  for (let height = start; height <= blockCount; height++) {
    console.log(
      `${height}/${blockCount} blocks (${((100 * height) / blockCount).toFixed(
        2
      )}%)`
    );
  ...
  for (const item of db.discovered ?? []) {
    ...
    await jsonPersister.updateDb((db) => {
      ...
      const idx = list.findIndex((d) =>
        sameItem(keyFromItem(d), keyFromItem(update))
      );
      if (idx >= 0) {
        list[idx] = update;
      }
...
async function getAccountXprvsFromCore(): Promise<AccountKeys> {
  ...
  for (const { desc } of descriptors) {
    const xprvMatch = desc.match(
      /\((?:\[.*?])?([xt]prv[1-9A-HJ-NP-Za-km-z]+)(?:\/(\d+)h\/(\d+)h\/(\d+)h)?/
    );
    if (!xprvMatch) {
      continue;
    }
  ...
  const isTestnet = ["testnet", "signet", "regtest"].includes(
...
async function getUnspentOutputs() {
  const unspentOutputs = (await btcClient.listUnspent()).sort(
...
async function checkPendingTransactions(): Promise<boolean> {
  ...
  if (db.pendingTaproot.revealTxids?.length) {
    ...
    if (mined >= 0) {
      ...
      console.log(
        "pendingTaproot revealTxid",
        db.pendingTaproot.revealTxids.at(-1)
      );
    }
  }
...
async function replaceByFee(): Promise<boolean> {
  ...
  const { entry: revealEntry, txid: revealTxid } = await getEntryFromMempool(
    db.pendingTaproot.revealTxids
  );
  const revealHex = (await btcClient.getRawTransaction(
    revealTxid,
    0
  )) as string;
  const commitHex = await extractCommitHex(revealHex);

  const feeResp = await btcClient.estimateSmartFee(
    config.feeConf,
    SMART_FEE_MODE
  );
  const estSatPerVByte = feeResp.feerate
    ? Math.ceil(feeResp.feerate * 1e5)
    : config.feeFallback;
...
async function fundWalletMessage() {
  ...
  if (walletInfo.balance < config.feeMax) {
    ...
    console.log(
      `Wallet has insufficient funds (${walletInfo.balance}). Send ${config.chain} to ${address}`
    );
  }
}
...
async function anchorBatch(): Promise<void> {
  ...
  try {
    ...
    const { commitHex, revealHex, batch } =
      await inscription.createTransactions(
        payload,
        tapInfo.hdkeypath,
        utxos,
        estSatPerVByte,
...
async function importLoop(): Promise<void> {
  if (importRunning) {
    ...
    console.log(
      `import loop busy, waiting ${config.importInterval} minute(s)...`
    );
    return;
  }
  ...
  try {
    ...
    console.error(
      `Error in importLoop: ${error.error || JSON.stringify(error)}`
    );
  } finally {
    importRunning = false;
...
async function exportLoop(): Promise<void> {
  if (exportRunning) {
    ...
    console.log(
      `Export loop busy, waiting ${config.exportInterval} minute(s)...`
    );
    return;
  }
...
async function waitForChain() {
  ...
  console.log(
    `Connecting to ${config.chain} node on ${config.host} using wallet '${config.wallet}'`
  );
...
async function main() {
  ...
  if (config.importInterval > 0) {
    console.log(
      `Importing operations every ${config.importInterval} minute(s)`
    );
    setTimeout(importLoop, config.importInterval * 60 * 1000);
  }
  ...
  if (config.exportInterval > 0) {
    console.log(
      `Exporting operations every ${config.exportInterval} minute(s)`
    );
    console.log(
      `Txn fees (${REGISTRY}): conf target: ${config.feeConf}, maximum: ${config.feeMax}, fallback Sat/Byte: ${config.feeFallback}`
    );
    setTimeout(exportLoop, config.exportInterval * 60 * 1000);
  }
}
