  ...
  for (const item of db.discovered) {
    try {
      ...
      if (error.error !== "DID not found") {
        console.error(
          `Error importing ${item.did}: ${error.error || JSON.stringify(error)}`
        );
      }
    }
  }
...
export async function createOpReturnTxn(
  opReturnData: string
...
async function getEntryFromMempool(
  txids: string[]
): Promise<{ entry: MempoolEntry; txid: string }> {
  if (!txids.length) {
...
async function replaceByFee(): Promise<boolean> {
  ...
  if (
    !db.pending?.txids ||
    !(await checkPendingTransactions(db.pending.txids))
  ) {
    return false;
  }
  ...
  const fee_rate =
    version < 210000 ? toEightDpCeil(targetSatPerVb * 1e-5) : targetSatPerVb;
...
async function anchorBatch(): Promise<void> {
  ...
  if (batch.length > 0) {
    ...
    const did = await keymaster.createAsset(
      { batch },
      { registry: "hyperswarm", controller: config.nodeID }
    );
...
async function main() {
  ...
  if (config.exportInterval > 0) {
    ...
    console.log(
      `Txn fees (${config.chain}): conf target: ${config.feeConf}, maximum: ${config.feeMax}, fallback Sat/Byte: ${config.feeFallback}`
    );
    setTimeout(exportLoop, config.exportInterval * 60 * 1000);
  }
}
